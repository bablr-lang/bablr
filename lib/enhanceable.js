...
import { evaluateReturn, get, list, symbolName } from '@bablr/agast-helpers/tree';
...
export { treeFromStream, streamFromTree } from '@bablr/agast-helpers/stream';
    ...
    return new Proxy(defaultTag, {
      ...
      get(_, name) {
        return (quasis, ...expressions) => {
          let { hasGap } = getEmbeddedCallable(defaultMatcher).nodeMatcher.value.flags;

          let matcher = buildEmbeddedCallable(
            freezeRecord({
              reference: null,
              bindings: getEmbeddedCallable(defaultMatcher).bindings,
          ...
          if (!(tree = trees.get(quasis))) {
            tree = treeParse(language, matcher, sourceFromQuasis(quasis), props, options);
            trees.set(quasis, tree);
          }
